...
Ainsi, une discussion est entièrement retirée du jeu de données s'il manque la \emph{racine}, si un de ses messages a été retiré à l'étape précédente ou si la discussion a débuté trop récemment ou si elle dure trop longtemps.
...
La limite pour considérer une discussion trop récente ou trop longue a été fixé à 2 ans ($6.3\times 10^7 s$) en observant la distribution de durées des discussions, voir la figure~\ref{fig:dists_discussion}.
\inline{Refaire les courbes qui sont avant la suppression à cause de la durée.}
...
La distribution des durées des discussions montre que la majorité des discussions dure environ une journée ou moins (100000 secondes équivaut à moins de 28 heures).
...
La définition de $\delta^{inter}_{\Delta}(D_i,D_j)$ est la suivante: $\delta^{inter}_{\Delta}(D_i,D_j) = \delta(L_{t_{\beta}'..t_{\psi}'}'(V'))$.
...
Le flot quotient est plus $\Delta$-dense que le flot initial pour des valeurs de $\Delta$ importantes.
