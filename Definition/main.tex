...
Les liens de $E$ sont des quadruplets $(b,e,u,v)$, signifiant que la paire de n\oe uds $(u, v)$ existe sur l'intervalle $[b,e] \subseteq [\alpha,\omega]$.
Nous dénotons le nombre de liens dans le flot par $|L|=|E|=m$ la durée du flot par $\bar{L}=\omega-\alpha$.
...
Un graphe $G=(V,E')=G(L)$ est le graphe agrégé de $L$ si $\forall (u,v) \in E',\ \exists b,e \in T$ tel que $(b,e,u,v) \in E$.
...
Cette notion est en particulier utile pour définir des sous-flots induits par différents ensemble d'éléments.
...
Nous définissons $L(S)$ , le sous-flot de $L$ induit par un ensemble de paires n\oe uds $S \in V'^2$: $L(S)=([\beta(E'),\psi(E')],V',E')$ avec $E'= \{(b,e,u,v) \in E, (u,v) \in S\}$ et $V' \subset V$.
...
Cela peut être fait rapidement si chaque n\oe ud a la connaissance des liens auxquels il est relié.
...
Pour l'intervalle de temps, la situation est plus compliquée car il n'est pas facile de connaitre l'ensemble des liens existent à un instant donné.
Il est uniquement possible de les trier par ordre d'apparition ou de disparition.
Il n'y a pas d'ordre total sur la présence qui permettrait de considérer uniquement les liens appartenant au sous-flot considéré.
Pour qu'un lien, $l= (b,e,u,v)$ appartienne au sous-flot temporel sur $[t,t']$, il doit remplir une des ces conditions non exclusives:
\begin{itemize}
\item \textbf{$t \leq b \leq t'$} le lien commence dans l'intervalle $[t,t']$.
\item \textbf{$t \leq e \leq t'$} le lien fini dans l'intervalle $[t,t']$.
\item \textbf{$b \leq t \, \wedge \, t' \leq e$} le lien couvre tout l'intervalle $[t,t']$.
\end{itemize}
...
Il n'est pas possible de restreindre d'avantage le parcours car l'ensemble des liens débutants entre $\alpha$ et $t$ peuvent être dans le sous-flot en respectant la dernière condition.
...
\begin{equation}
d(t, V') = d_t(V')= \sum_{v \in V'} d_t(v) = 2 |L_{t}(V'\,^2)| \,,
\end{equation}

\begin{equation}
d(t, E') = d_t(E')=2|L_{t}(E')| \,.
\end{equation}

Avec ces définitions, il est aussi possible définir les notions de degré temporel interne, $d_t^{in}$, et externe $d_t^{out}$ de manière analogue à ce qui est fait dans les graphes.
Les degrés temporels d'un n\oe uds, d'un ensemble de n\oe uds ou de liens peuvent se calculer rapidement.
Il faut pour ce faire considérer les liens appartenant au sous-flot induit respectivement par un n\oe uds, ensemble de liens ou un ensemble de paires de n\oe uds.
...
Ainsi, le degré temporel à un instant $t$ est égale à la valeur de la somme des modification à cet instant.
\bigskip

Les degrés sont des fonctions du temps mais il est souvent intéressant de regarder la valeur moyenne du degré sur un intervalle donné:
\begin{equation}
d_{t..t'}(u)=\dfrac{1}{t'-t}  \int_{t}^{t'}d(u,t) dt = \dfrac{D_{t..t'}(u)}{t'-t} = \dfrac{\sum_{l \in L_{t..t'}(u)}\bar{l}}{t'-t} \, .
\label{eq:deg_moyen}
\end{equation}
...
Ils considèrent l'ajout de temps arbitraire sur chaque lien
Leur formulation est équivalente à la densité suivante: $\delta( \sigma(\xi(L,\Delta)))$ si l'augmentation de la durée ne se fait pas de manière symétrique mais en uniquement en diminuant le temps de début.
...
Le code de cette implémentation est en ligne\,\footnote{\url{XXX}}\note{A Mettre} et la documentation également\,\footnote{\url{XXX}}\note{A Mettre}.
...
C'est pourquoi, nous utilisons un algorithme probabiliste qui teste des permutations aléatoire.
