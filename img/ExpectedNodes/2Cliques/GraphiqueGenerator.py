...
def boxGraph():
	for fPath in  sys.argv[2:]:
		with open(fPath, 'r') as f:
			...
			fig.savefig('LFR1_'+name+'_ALL.eps',format="eps", bbox_inches='tight',dpi=1200)
			plt.close()
...
def FctGraph():
	for fPath in  sys.argv[2:]:
		with open(fPath, 'r') as f:
			...
			ax.set_xlabel("p", fontsize=30) # Overlap group size
			if name == "Ahn":
				name = "Partition_density"
			elif name == "Modified":
				name ="Expected_Nodes"
			elif name == "Evans1":
				name = "Evans1"
			...
			for k in toPrint:
				...
				if count < midleToprint or k == "2" :
					...
					p1,=plt.plot(tab['X'],tab[k],'--',zorder =10-count,linewidth=2.5)
					col1+=1
				else:
			...
			# setFigLinesBW(fig)
			fig.savefig('2Clique_'+name.strip(" ")+'.eps',format="eps", bbox_inches='tight',dpi=1200)
			plt.close()
	exit(1)
